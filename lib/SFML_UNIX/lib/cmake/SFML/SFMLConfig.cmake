# This script provides the SFML_UNIX libraries as imported targets
# ------------------------------------
#
# Usage
# -----
#
# When you try to locate the SFML_UNIX libraries, you must specify which modules you want to use (system, window, graphics, network, audio, main).
# If none is given, no imported target will be created and you won't be able to link to SFML_UNIX libraries.
# example:
#   find_package(SFML_UNIX COMPONENTS graphics window system) # find the graphics, window and system modules
#
# You can enforce a specific version, either MAJOR.MINOR or only MAJOR.
# If nothing is specified, the version won't be checked (i.e. any version will be accepted).
# example:
#   find_package(SFML_UNIX COMPONENTS ...)     # no specific version required
#   find_package(SFML_UNIX 2 COMPONENTS ...)   # any 2.x version
#   find_package(SFML_UNIX 2.6 COMPONENTS ...) # version 2.6 or greater
#
# By default, the dynamic libraries of SFML_UNIX will be found. To find the static ones instead,
# you must set the SFML_STATIC_LIBRARIES variable to TRUE before calling find_package(SFML_UNIX ...).
# You don't need to deal with SFML_UNIX's dependencies when linking your targets against SFML_UNIX libraries,
# they will all be configured automatically, even if you use SFML_UNIX static libraries.
# example:
#   set(SFML_STATIC_LIBRARIES TRUE)
#   find_package(SFML_UNIX 2 COMPONENTS network system)
#
# On macOS by default CMake will search for frameworks. If you want to use static libraries and have installed
# both SFML_UNIX frameworks and SFML_UNIX static libraries, your must set CMAKE_FIND_FRAMEWORK to "NEVER" or "LAST"
...
#
# Additionally, keep in mind that SFML_UNIX frameworks are only available as release libraries unlike dylibs which
# are available for both release and debug modes.
#
# If SFML_UNIX is not installed in a standard path, you can use the SFML_DIR CMake variable
# to tell CMake where SFML_UNIX's config file is located (PREFIX/lib/cmake/SFML_UNIX for a library-based installation,
# and PREFIX/SFML_UNIX.framework/Resources/CMake on macOS for a framework-based installation).
#
# Output
# ------
#
...
# - For each specified module XXX (system, window, graphics, network, audio, main):
#   - sfml-XXX
# The SFML_UNIX targets are the same for both Debug and Release build configurations and will automatically provide
# correct settings based on your currently active build configuration. The SFML_UNIX targets name also do not change
# when using dynamic or static SFML_UNIX libraries.
#
# When linking against a SFML_UNIX target, you do not need to specify indirect dependencies. For example, linking
# against sfml-graphics will also automatically link against sfml-window and sfml-system.
#
# example:
#   find_package(SFML_UNIX 2 COMPONENTS graphics audio REQUIRED)
...
# Generate imported targets for SFML_UNIX and its dependencies
