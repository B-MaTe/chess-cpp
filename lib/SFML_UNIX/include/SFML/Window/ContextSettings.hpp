...
/// SFML_UNIX rendering (graphics module), so you may need to use
/// this structure only if you're using SFML_UNIX as a windowing
...
/// are not supported by the system; instead, SFML_UNIX will try to
